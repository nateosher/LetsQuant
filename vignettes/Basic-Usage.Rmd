...
The result is a tibble with four columns: `a`, `b`, `distribution`, and `selection_counts`. The first three store information about the CDF function used; the final column is the selection count as previously described. 

The goal of using a quantlet basis is to explain as much of the variance using as small a number of quantlets as possible. So lets only use quantlets that were selected at least 8 times across all data entries. We can do this by passing the `selection_count_tib` to the `SelectQuantlets` function. The second argument determines the threshold at which a particular quantlet is included in the basis. If a quantlet is selected at least that many times, it is included in the basis.  

```{r}
quantlet_matrix = SelectQuantlets(selection_count_tib, 8)

class(quantlet_matrix)
dim(quantlet_matrix)
```

The result is a matrix where each column represents a selected quantlet, and each entry in that column represents the corresponding quantile function evaluated on a grid between 0 and 1. We can see from the dimensions of the result that 7 quantlets were selected at least 8 times each. As for the number of rows, the size of the grid defaults to 1,024, so each column is $Q(\frac{1}{1,025}), \dots, Q(\frac{1,024}{1,025})$ for a given quantile function.  Except there's a small catch, which you'll notice if you actually plot the resulting quantlet basis:

```{r}
PlotQuantletMatrix(quantlet_matrix)
```

These don't look like CDFs. But this is just because `SelectQuantlets` automatically orthogonalizes the resulting basis, which is almost always desirable. But if you'd like to override this behavior, you can set the `orthogonalize` argument in `SelectQuantlets` to `FALSE`. 

```{r}
unorthogonalized_quantlet_matrix = SelectQuantlets(selection_count_tib, 8, 
                                                   orthogonalize = FALSE)
...
The result is a list where each entry is a numeric vector with length equal to the number of quantlets in the basis. Each entry corresponds to the data entry at that index, and represents the coefficients of the data in "quantlet space." Thus, in order to view the projected data, we can simply multiply these coefficients by the quantlet matrix we constructed eariler. More concretely, here's how the first entry looks when projected:
...
This looks pretty good. However, you might be interested in a more rigorous assessment. After all, we chose the threshold of 8 somewhat arbitrarily- why not look at how different thresholds impact the "quality" of the projection?
